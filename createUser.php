<?php
	...
	if($result_array['status'] !== 1 or strcmp($role,"superUser")!==0){
		echo "<meta http-equiv='refresh' content='0;url=" . $config['uri_int'] . "'>";
	}

	$result = file_get_contents($config['uri_api'] . "/getGraphOwnerUsers?session=" . $_GET["session_id"], false, $context);
...
<div class="container">
		...
		<table id="events-id2" data-url="data1.json" data-height="299" data-search="true" class="table table-hover">
		    ...
		    <tbody>
		    	...
		    	<tr data-index="0" id="<?php echo "users_" . $n; ?>">
		    		...
		    		<td style="">
	    				<a class="remove ml10" onclick="<?php echo 'deleteGraphOwnerUser(\''.$key1.'\','.$n.')';?>" title="Remove">
	...
	<div class="row clearfix">
		...
		<div class="col-md-12 column">
			...
			<button type="button" class="btn btn-default" onclick="createGraphOwnerUser();">Create</button>
			<label id="msg"></label>
		</div>
	</div>
</div>
